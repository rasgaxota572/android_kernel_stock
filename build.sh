...
function KERNEL_COMPILE() {
	...
	# Make the config
	make O=out ARCH=arm64 sunburn-sunburn-guamp_defconfig
...
# Disable some config
cp arch/arm64/configs/sunburn-sunburn-guamp_defconfig arch/arm64/configs/sunburn-guamp_defconfig.bak
sed -i 's/^CONFIG_CAMERA_BOOTCLOCK_TIMESTAMP=.*/# CONFIG_CAMERA_BOOTCLOCK_TIMESTAMP is not set/' arch/arm64/configs/sunburn-guamp_defconfig

# Run functions for R variant
KERNEL_COMPILE "$1"
KERNEL_RESULT anykernelR "$KERNEL_NAME1"
mv arch/arm64/configs/sunburn-guamp_defconfig.bak arch/arm64/configs/sunburn-guamp_defconfig
